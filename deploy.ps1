...
#   wrangler login              # 弹出浏览器授权 Cloudflare
# 之后每次改完代码运行本脚本即可，10 秒部署完成。
# 部署 URL: https://triumph.pages.dev
...
if ($LASTEXITCODE -ne 0) {
  Write-Host "需要登录 Cloudflare，浏览器将弹出授权..." -ForegroundColor Yellow
  wrangler login
}

# 3. 部署 site/ 到 Pages（项目名 triumph-6eq；--commit-dirty=true 避免 git 无 HEAD 时误判文件未变）
Write-Host "Deploying..." -ForegroundColor Cyan
Push-Location $proj
wrangler pages deploy $site --project-name triumph-6eq --commit-dirty=true
$code = $LASTEXITCODE
Pop-Location
