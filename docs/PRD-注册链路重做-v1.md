...
## 11. 决策冻结（2026-09-03 23:50，老户拍板）

| # | 决策 | 状态 |
| --- | --- | --- |
| Q1 | **接受双轨**（magic link + 6 位验证码）。附加要求：**邮件与结果页均加提示"建议直接从邮箱里打开链接"**，减少移动端切 App 断点 | ✅ 已冻结 |
| Q2 | **接受无密码注册**。老户原话："反正现在也没有注册用户（也有可能注册了我看不到）"——注意上线后用 D4 埋点核实注册漏斗，顺带回答这个问题 | ✅ 已冻结 |
| Q3 | **改动窗口今天开启**。工作树已清零（85a970f 交付物 + a6101b0 检查点，老户批准提交），实施从干净树开始 | ✅ 已开窗 |
| Q4 | **邮件走 Resend**（老户费力配置过，除非 Mailgun 能一键配置——未提供，故定 Resend） | ✅ 已冻结 |

实施顺序（窗口期内）：
1. 修 `hVerify` state 覆盖缺陷（§4.3，读-改-写 + pending payload 合并）——最高优先
2. worker 端 magic link 四端点 + Resend 发信（含 Q1 提示文案）
3. 前端结果页内联注册卡片（§5 交互规格）
4. D4 埋点层接入（`drafts/2026-09-03-tracking-layer-draft.js` → `site/js/tracking.js`，worker `/api/t/events` 接收）
5. 全链路自测（本地 wrangler dev + mock 邮件），部署需老户另行确认
